...
def draw_violin(data, label_list, cellline):
    ...
    sns.boxplot(x="label",  # 指定x轴的数据
                   ...
                   flierprops=dict(markerfacecolor='black', markeredgecolor='black', markersize=1)  # 设置离群点的颜色
                   )
...
if __name__ == '__main__':
    ...
    for i in range(4):
        # dhit2
        dhit2_corr_data = np.expand_dims(excel_data(excel_file, row, 2 * i + 1, table_id, lines=lines), axis=-1)
        ...
        # dhit
        dhit_corr_data = np.expand_dims(excel_data(excel_file, row, 2 * i + 11, table_id, lines=lines), axis=-1)
