...
class Dhit2(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self._trunk_promoter = Sequential([transformer_promoter,
                            crop_final,
                            ],
                             name='trunk_promoter')
    ...
    with tf.name_scope('heads'):
      self._heads_promoter = Sequential([final_pointwise_promoters,
                                tf.keras.layers.Dense(units=2), SoftPlus()
                                ], name=f'head_promoter')
      
      self._heads_others = Sequential([final_pointwise_others,
                                tf.keras.layers.Dense(units=output_channels-2), SoftPlus()
                                ], name=f'head_others')
  ...
  @property
  def trunk_promoter(self):
    return self._trunk_promoter

  @property
  def heads_promoter(self):
    return self._heads_promoter
  
  @property
  def heads_others(self):
    return self._heads_others
  ...
  def __call__(self, inputs, is_training) -> Dict[str, tf.Tensor]:
    ...
    outputs_others = self.trunk(outputs, training=is_training)

    outputs_promoter = self.trunk_promoter(outputs, training=is_training)



    outputs_promoter = self.heads_promoter(outputs_promoter, training=is_training)

    outputs_others = self.heads_others(outputs_others, training=is_training)

    outputs = tf.concat([outputs_promoter, outputs_others], axis=-1)

    # outputs = tf.cast(outputs > 0.1, dtype=tf.float32)
