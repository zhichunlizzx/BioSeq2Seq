...
class BioSeq2Seq(tf.keras.Model):
  ...
  def __init__(self,
    ...
    stem_proseq_minus = Sequential([
        tf.keras.layers.Conv1D(filters=channels // 2, kernel_size=15, padding="same"),
        Residual(conv_block(channels // 2, 1, name='pointwise_conv_block_minus')),
        pooling_module(pooling_type, pool_size=2),
    ], name='stem_proseq_minus')

    stem_proseq_plus = Sequential([
        tf.keras.layers.Conv1D(filters=channels // 2, kernel_size=15, padding="same"),
        Residual(conv_block(channels // 2, 1, name='pointwise_conv_block_plus')),
        pooling_module(pooling_type, pool_size=2),
    ], name='stem_proseq_plus')

    stem_proseq_minus_plus = Sequential([
        tf.keras.layers.Conv1D(filters=channels // 2, kernel_size=15, padding="same"),
        Residual(conv_block(channels // 2, 1, name='pointwise_conv_block_minus_plus')),
        pooling_module(pooling_type, pool_size=2),
    ], name='stem_proseq_minus_plus')
    ...
    conv_tower_proseq_minus = Sequential([
        Sequential([
            conv_block(num_filters, 5),
            Residual(conv_block(num_filters, 1, name='pointwise_conv_block_minus')),
            pooling_module(pooling_type, pool_size=2),
            ],
                   name=f'conv_tower_block_minus_{i}')
        for i, num_filters in enumerate(filter_list)], name='conv_tower_proseq_minus')

    conv_tower_proseq_plus = Sequential([
        Sequential([
            conv_block(num_filters, 5),
            Residual(conv_block(num_filters, 1, name='pointwise_conv_block_plus')),
            pooling_module(pooling_type, pool_size=2),
            ],
                   name=f'conv_tower_block_plus_{i}')
        for i, num_filters in enumerate(filter_list)], name='conv_tower_proseq_plus')

    conv_tower_proseq_minus_plus = Sequential([
        Sequential([
            conv_block(num_filters, 5),
            Residual(conv_block(num_filters, 1, name='pointwise_conv_block_minus_plus')),
            pooling_module(pooling_type, pool_size=2),
            ],
                   name=f'conv_tower_block_minus_plus_{i}')
        for i, num_filters in enumerate(filter_list)], name='conv_tower_proseq_minus_plus')
    ...
    self._conv_minus = Sequential([stem_proseq_minus,
                            conv_tower_proseq_minus],
                            name = 'conv_block_minus')

    self._conv_plus = Sequential([stem_proseq_plus,
                            conv_tower_proseq_plus],
                            name = 'conv_block_plus')

    self._conv_minus_plus = Sequential([stem_proseq_minus_plus,
                            conv_tower_proseq_minus_plus],
                            name = 'conv_block_minus_plus')
  ...
  @property
  def conv_minus(self):
    return self._conv_minus

  @property
  def conv_plus(self):
    return self._conv_plus
  ...
  def __call__(self, inputs: tf.Tensor, proseq_minus: tf.Tensor, proseq_plus: tf.Tensor, proseq_minus_plus: tf.Tensor,
